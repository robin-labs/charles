...
echo_sources = ECHO_SOURCES


def write_differential_scenes():
    normalized = render_scene(fs, device, chirp, head, echo_sources, 1 / 0.03)
    unnormalized = render_scene(fs, device, chirp, head, echo_sources, 1)
    write_scene(fs, normalized, "output/diff/normalized.wav")
    write_scene(fs, unnormalized, "output/diff/unnormalized.wav")
